...
function AdminEmailGenerator() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (fetchedLeads.length > 0 && fetchedUsers.length > 0) {
          setFormData({
            ...formData,
            leadId: fetchedLeads[0]._id,
            sentBy: fetchedUsers[0]?._id || userId,
          });
        }
      } catch (err) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow">
      <h2 className="text-3xl font-bold mb-4">Email Generator (Admin)</h2>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <button
            ...
            className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition col-span-3"
            disabled={leads.length === 0 || !formData.description || !formData.sentBy}
          >
            Generate Email
          </button>
        </div>
